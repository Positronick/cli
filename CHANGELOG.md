...
- **`grok` soul install target**: `positronick soul install <slug> --target grok` writes `~/.grok/SOUL.md`; `--target grok --link` appends an instruction line (not an `@`-import — Grok Build does not expand them) to `$GROK_HOME/AGENTS.md` so Grok Build actually loads the persona. Harness detection also picks up a `.grok` marker directory, last in priority so it never outranks an existing harness.
...
- **`soul install --target openclaw` now writes to the real OpenClaw workspace**: OpenClaw reads `SOUL.md` from its configured agent's workspace directory, not `~/.openclaw`, so the install (and `init`) resolve the destination from `openclaw.json`/env, defaulting to `~/.openclaw/workspace/SOUL.md`. New `--workspace <agent-id|dir>` flag picks the agent when several are configured; a multi-agent setup without it fails loudly instead of guessing.
- **`positronick skill install <slug>` installs the hosted SKILL.md**: when a skill listing's `hasAsset` is true, `skill install` now fetches and writes its hosted `SKILL.md` verbatim (the same download-counted contract as `soul install`) instead of printing the install command. New flags `--target agents|claude|cursor|grok|codex|openclaw` (default `agents`, the shared standard), `--project` (project-local variant under cwd), `--path <dir>`, and `--force`; `--run`/`--yes` are rejected for a hosted skill. The install folder is always the listing's slug, and the fetched SKILL.md's frontmatter `name` must equal it or the install fails with nothing written. Skills with no hosted asset keep the existing print/`--run` behavior unchanged. New MCP tool **`skill_install`** mirrors `soul_install`'s never-overwrite rule — the server now advertises six tools.
